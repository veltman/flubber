...
tape("all with string inputs", function(test) {
  let interpolate = all([square, halfcircle], [bezier, square], { maxSegmentLength: 10 }),
    interpolateArray = all([square, halfcircle], [bezier, square], {
      maxSegmentLength: Infinity,
      string: false
    });
...
tape("all with string inputs single", function(test) {
  let interpolate = all([square, halfcircle], [bezier, square], {
    single: true
  }),
    interpolateArray = all([square, halfcircle], [bezier, square], {
      maxSegmentLength: Infinity,
...
tape("matching", function(test) {
  ...
  let interpolate = all([leftSquare, rightSquare], [rightTriangle, leftTriangle]),
    interpolateNoMatch = all([leftSquare, rightSquare], [rightTriangle, leftTriangle], {
      match: false
    }),
    interpolateSingle = all([leftSquare, rightSquare], [rightTriangle, leftTriangle], {
      single: true
    }),
    interpolateSingleNoMatch = all([leftSquare, rightSquare], [rightTriangle, leftTriangle], {
      single: true,
      match: false
    });
